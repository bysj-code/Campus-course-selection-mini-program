	...
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/course_schedule/table','add') || $check_action('/course_schedule/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/course_schedule/table','del') || $check_action('/course_schedule/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				<el-table-column prop="course_no" @sort-change="$sortChange" label="课程编号"
				v-if="user_group == '管理员' || $check_field('get','course_no')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','cover')" min-width="200">
					...
					<el-table-column prop="course_type" @sort-change="$sortChange" label="课程类型"
				v-if="user_group == '管理员' || $check_field('get','course_type')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','class_location')" min-width="200">
					...
					<el-table-column prop="teacher_user" @sort-change="$sortChange" label="教师用户"
				v-if="user_group == '管理员' || $check_field('get','teacher_user')" min-width="200">
						<template slot-scope="scope">
					{{ get_user_teacher_user(scope.row['teacher_user']) }}
				</template>
					</el-table-column>
					<el-table-column prop="teacher_name" @sort-change="$sortChange" label="教师姓名"
				...
				v-if="user_group == '管理员' || $check_field('get','student_name')" min-width="200">
					...
					<el-table-column prop="confirm_the_number_of_people" @sort-change="$sortChange" label="确认人数"
				v-if="user_group == '管理员' || $check_field('get','confirm_the_number_of_people')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/course_schedule/table','set') || $check_action('/course_schedule/view','set') || $check_action('/course_schedule/view','get') || $check_action('/${table.tableConfig.table_nav_name}/table','add') || $check_action('/${table.tableConfig.table_nav_name}/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/course_schedule/table','set') || $check_action('/course_schedule/view','set') || $check_action('/course_schedule/view','get')"
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/course_schedule/get_list?like=0",
				url_del: "~/api/course_schedule/del?",
