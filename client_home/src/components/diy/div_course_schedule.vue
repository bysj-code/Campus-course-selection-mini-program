<template>
	<view class="diy_details diy_div_course_schedule">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','confirm_the_number_of_people')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>确认人数:</span>
						</view>
							<view class="diy_field diy_text">
							<text>
								{{obj["confirm_the_number_of_people"]}}
							</text>
						</view>
						</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/course_schedule/get_obj", {
					course_schedule_id:this.query.course_schedule_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["course_schedule_id"] !==0) {
