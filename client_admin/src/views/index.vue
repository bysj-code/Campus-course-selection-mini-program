...
<!--					</el-col>-->
				...
				<el-row>
								<el-col v-if="user_group == '管理员' || $check_figure('/course_information/table')" :span="8">
						<div class="card chart">
									<newBarChart v-if="bar_obj_course_information.values.length > 0" id="bar_obj_course_information" :vm="bar_obj_course_information" :title="'课程信息统计'">
							</newBarChart>
							<div v-if="!bar_obj_course_information.values.length">课程信息没有符合条件的数据</div>
								</div>
					</el-col>
							</el-row>
...
<script>
	...
	export default {
		...
		created() {
						// 执行课程信息数据获取
			this.get_list_course_information();
					},
		mounted() {},
		methods: {
				...
				if (flag) {
					...
					for (let i=0;i<list.length;i++){
						await this.$get(
								...
								// 获取课程信息统计柱状图
			async get_list_course_information() {
				let name_list = [];
				let query_str = "";
								...
								let flag = false;
												let date_flag = "其他"
																																																						name_list.push("剩余人数");
				query_str = query_str+"number_of_people_remaining"+","
																																	this.bar_obj_course_information.names = name_list
				...
				if (user_group!='管理员'){
						...
						await this.$get(
						"~/api/course_information/bar_group?field="+query_str+"&groupby="+group_by_value,
						data,
						(json) => {
							if (json.result) {
								let xAxis = [];
								...
								this.bar_obj_course_information.xAxis = xAxis;
								this.bar_obj_course_information.values = values;
							}
							if (flag){
								this.get_nickname(this.bar_obj_course_information.xAxis,true);
							}
						});
			},
