<template>
	<view class="page_course_information diy_list diy_list_page div_list_tml" id="course_information_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
														<uni-search-bar placeholder="搜索课程名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'course_name')">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																			<uni-section title="课程类型" type="line">
							<uni-data-select
									@change="change_course_type"
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?course_information_id=' + o['course_information_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
		...
		data() {
			return {
				url_get_list: "~/api/course_information/get_list?like=0",
											// 课程类型选项列表
				list_course_type: [{value:"全部",text:"全部"}],
															// 用户列表
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=course_information", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["course_information_id"] === o["source_id"]){
				...
				console.error(res.error);
						...
						// 改变分类标签
	change_course_type(val) {
		if (val === "全部" || val === "") {
			this.query.course_type = "";
		}else{
			this.query.course_type = val
		}
		this.search();
	},
