          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','course_no') || $check_field('add','course_no') || $check_field('set','course_no')" label="课程编号" name="course_no">
                                <uni-easyinput type="text" v-model="form['course_no']" v-if="user_group === '管理员' || (form['course_information_id'] && $check_field('set','course_no')) || (!form['course_information_id'] && $check_field('add','course_no'))"  :disabled="true" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','course_name') || $check_field('add','course_name') || $check_field('set','course_name')" label="课程名称" name="course_name">
                                <uni-easyinput type="text" v-model="form['course_name']" v-if="user_group === '管理员' || (form['course_information_id'] && $check_field('set','course_name')) || (!form['course_information_id'] && $check_field('add','course_name'))" :disabled="disabledObj['course_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','course_type') || $check_field('add','course_type') || $check_field('set','course_type')" label="课程类型" name="course_type">
                  ...
                  :localdata="list_course_type"
                  :clear="!disabledObj['course_type_isDisabled']"
              ...
              <uni-forms-item v-if="$check_field('get','class_time') || $check_field('add','class_time') || $check_field('set','class_time')" label="上课时间" name="class_time">
                                <uni-easyinput type="text" v-model="form['class_time']" v-if="user_group === '管理员' || (form['course_information_id'] && $check_field('set','class_time')) || (!form['course_information_id'] && $check_field('add','class_time'))" :disabled="disabledObj['class_time_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','class_location') || $check_field('add','class_location') || $check_field('set','class_location')" label="上课地点" name="class_location">
                                <uni-easyinput type="text" v-model="form['class_location']" v-if="user_group === '管理员' || (form['course_information_id'] && $check_field('set','class_location')) || (!form['course_information_id'] && $check_field('add','class_location'))" :disabled="disabledObj['class_location_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','class_location')">
                  {{ form['class_location'] }}
                </text>
                            </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','number_of_people_remaining') || $check_field('add','number_of_people_remaining') || $check_field('set','number_of_people_remaining')" label="剩余人数" name="number_of_people_remaining">
                                <uni-easyinput type="text" v-model="form['number_of_people_remaining']" v-if="user_group === '管理员' || (form['course_information_id'] && $check_field('set','number_of_people_remaining')) || (!form['course_information_id'] && $check_field('add','number_of_people_remaining'))" :disabled="disabledObj['number_of_people_remaining_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','number_of_people_remaining')">
                  {{ form['number_of_people_remaining'] }}
                </text>
                            </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','teacher_user') || $check_field('add','teacher_user') || $check_field('set','teacher_user')" label="教师用户" name="teacher_user">
                  ...
                  v-if="user_group === '管理员' || (form['course_information_id'] && $check_field('set','teacher_user')) || (!form['course_information_id'] && $check_field('add','teacher_user'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['teacher_user']"
                  :localdata="list_user_teacher_user"
              ...
              <uni-forms-item v-if="$check_field('get','teacher_name') || $check_field('add','teacher_name') || $check_field('set','teacher_name')" label="教师姓名" name="teacher_name">
                                <uni-easyinput type="text" v-model="form['teacher_name']" v-if="user_group === '管理员' || (form['course_information_id'] && $check_field('set','teacher_name')) || (!form['course_information_id'] && $check_field('add','teacher_name'))" :disabled="disabledObj['teacher_name_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','teacher_name')">
                  {{ form['teacher_name'] }}
                </text>
                            </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','course_introduction') || $check_field('add','course_introduction') || $check_field('set','course_introduction')" label="课程介绍" name="course_introduction">
                        <uni-easyinput type="textarea" v-model="form['course_introduction']" v-if="user_group === '管理员' || (form['course_information_id'] && $check_field('set','course_introduction')) || (!form['course_information_id'] && $check_field('add','course_introduction'))" :disabled="disabledObj['course_introduction_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','course_introduction')">
                  {{ form['course_introduction'] }}
                </text>
                    </uni-forms-item>

              <uni-forms-item label="计时器标题" name="timer_title">
                <uni-easyinput v-if="user_group === '管理员' || (form['course_information_id'] && $check_action('/course_information/view','set')) || (!form['course_information_id'] && $check_action('/course_information/view','add'))" v-model="form['timer_title']" type="text" :disabled="disabledObj['timer_title_isDisabled']" />
                <text v-else-if="$check_field('get','timer_title')">
                  {{ form['timer_title'] }}
                </text>
              </uni-forms-item>
              <uni-forms-item label="计时开始时间" name="timing_start_time">
                <uni-datetime-picker v-if="user_group === '管理员' || (form['course_information_id'] && $check_action('/course_information/view','set')) || (!form['course_information_id'] && $check_action('/course_information/view','add'))" @change="changeLog($event,'timing_start_time')" v-model="form['timing_start_time']" type="datetime" :disabled="disabledObj['timing_start_time_isDisabled']" />
                <text v-else-if="$check_field('get','timing_start_time')">
                  {{ form['timing_start_time'] }}
                </text>
              </uni-forms-item>
              <uni-forms-item label="计时结束时间" name="timing_end_time">
                <uni-datetime-picker v-if="user_group === '管理员' || (form['course_information_id'] && $check_action('/course_information/view','set')) || (!form['course_information_id'] && $check_action('/course_information/view','add'))" @change="changeLog($event,'timing_end_time')" v-model="form['timing_end_time']" type="datetime" :disabled="disabledObj['timing_end_time_isDisabled']" />
                <text v-else-if="$check_field('get','timing_end_time')">
                  {{ form['timing_end_time'] }}
                </text>
              </uni-forms-item>
              <uni-forms-item label="限制次数" name="limit_times">
                <uni-easyinput v-if="user_group === '管理员' || (form['course_information_id'] && $check_action('/course_information/view','set')) || (!form['course_information_id'] && $check_action('/course_information/view','add'))" v-model="form['limit_times']" type="number" :disabled="disabledObj['limit_times_isDisabled']" />
                <text v-else-if="$check_action('/course_information/view','get')">
                  {{ form['limit_times'] }}
                </text>
              </uni-forms-item>
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/course_information/upload?'),
    ...
    async get_list_course_type() {
      ...
      if(json.result && json.result.list){
        json.result.list.map((o) => this.list_course_type.push({value:o.course_type,text:o.course_type}));
      }
      else if(json.error){
    ...
    async get_group_user_teacher_user() {
      ...
      if(json.result && json.result.obj){
        ...
        this.get_user_session_teacher_user(this.form['teacher_user'])
      }
      else if(json.error){
        console.error(json.error);
      }
