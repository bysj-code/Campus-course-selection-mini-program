<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="课程信息推荐"
      url="/pages/course_information/list"
      :list="list_article"
      v-if="$check_action('/course_information/list', 'get')"
    >
	  <list_course_information :list="list_course_information"></list_course_information>
		...
		*/
		get_course_information() {
			let url = "~/api/course_information/get_list?";
				...
				var url_get_list =
				"~/api/course_information/get_list?timing_start_time_max=" +
				oNowDate +
				"&timing_end_time_min=" +
				oNowDate +
				"&like=0";
			...
			this.$get(url, {
					...
					console.log("课程信息" ,json.result.list);
					this.list_course_information = json.result.list;
											this.list_course_information.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_course_information ,"course_information" ,"11058");
									}
			});
		},
