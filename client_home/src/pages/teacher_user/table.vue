<template>
  <view class="page_teacher_user diy_table diy_table_tml" id="teacher_user_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="教师姓名" name="teacher_name">
                      <uni-easyinput type="text" v-model="query.teacher_name" placeholder="教师姓名" />
                    </uni-forms-item>
                    <uni-forms-item label="教师性别" name="gender_of_teachers">
                 <uni-data-select placeholder="请选择"
                        v-model="query['gender_of_teachers']"
                        :localdata="list_gender_of_teachers"
       ></uni-data-select>
                          </uni-forms-item>
                </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/teacher_user/view?" v-show="user_group == '管理员' || $check_action('/teacher_user/table','add') || $check_action('/teacher_user/view','add')">添加</navigator>
                            ...
                            <span>教师姓名</span>
								...
								<text>
									{{ o["teacher_name"] }}
													...
													<view v-if="0 && $check_field('get','gender_of_teachers')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>教师性别</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/teacher_user/table','del') || $check_action('/teacher_user/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/teacher_user/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/teacher_user/table','set') || $check_action('/teacher_user/view','set') || $check_action('/teacher_user/view','get')" class="btn btn-primary">详情</navigator>
	    					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		data() {
			return {
				...
				list: [],
                            // 教师性别列表
                list_gender_of_teachers: [{value:"",text:"全部"}],
        			}
		},
		methods: {
          search() {
           ...
           * 获取教师性别列表
           */
          async get_list_gender_of_teachers() {
                      ['男','女'].map((o) => this.list_gender_of_teachers.push({value:o,text:o}));
                            },
            		},
		created() {
                      // 初始化教师性别列表
          this.get_list_gender_of_teachers();
        		}
	}
</script>
